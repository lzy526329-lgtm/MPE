import type { AccountError, AccountResult, GameAccountState, SaveSummary, SyncStatus } from '../electron/gameAccount/types'
...
function formMessage(message: string | null, tone: MessageTone = 'error'): string {
  if (!message) return ''
  const role = tone === 'error' ? 'alert' : 'status'
  return `<p class="account-message account-message--${tone}" role="${role}">${escapeHtml(message)}</p>`
}

function syncErrorText(error: AccountError): string {
  if (error.code === 'RATE_LIMITED') return '请求过于频繁，请稍后再试。'
  if (error.code === 'NETWORK_ERROR' || error.code === 'REQUEST_TIMEOUT' || error.code === 'SERVICE_UNAVAILABLE') {
    return '网络异常，进度已保存在本地，稍后会自动重试。'
  }
  if (/[\u4e00-\u9fff]/.test(error.message)) return error.message
  return '同步失败，请稍后重试。'
}

function syncFeedback(state: GameAccountState): { text: string; tone: MessageTone } {
  if (state.status === 'conflict') return { text: '发现存档冲突，请选择要保留的版本。', tone: 'info' }
  if (state.error) return { text: syncErrorText(state.error), tone: 'error' }
  if (state.status === 'offline-pending') return { text: '当前无法连上云端，进度已保存在本地，联网后会自动同步。', tone: 'info' }
  return { text: '同步成功。', tone: 'success' }
}
...
function authenticatedMarkup(state: GameAccountState, message: string | null, changingPassword: boolean, messageTone: MessageTone, syncPending: boolean): string {
  ...
  return `
    <section class="account-dashboard" aria-labelledby="account-dashboard-title">
      ...
      ${formMessage(message, messageTone)}
      ${conflict ? `
      ...
      ` : `
        <section class="account-sync-panel">
          ...
          <button class="secondary-button" type="button" data-account-action="sync-now"${state.status === 'syncing' || syncPending ? ' disabled' : ''}>${syncPending || state.status === 'syncing' ? '正在同步…' : '立即同步'}</button>
        </section>
      `}
...
export function mountAccountPage(): void {
  ...
  let messageTone: MessageTone = 'error'
  let countdown = 0
  ...
  const notice = (text: string | null, tone: MessageTone = 'error') => {
    message = text
    messageTone = tone
  }
  ...
  const render = () => {
    ...
    if (state?.account) {
      root.innerHTML = authenticatedMarkup(state, message, changingPassword, messageTone, syncPending)
      restoreDraft()
      return
    }
  ...
  const sendCode = async (purpose: 'register' | 'reset_password') => {
    ...
    try {
      ...
      startCountdown()
      notice('验证码已发送，请查收邮箱。', 'success')
      render()
    } catch {
      if (!isCurrentEmailCodeRequest(requestGeneration)) return
      sendingCode = false
      notice('验证码暂时不可用，请稍后重试。')
      render()
    }
  }
  ...
  root.addEventListener('click', (event) => {
    ...
    if (action === 'show-login') show('login')
    ...
    else if (action === 'show-change-password') { changingPassword = true; notice(null); render() }
    else if (action === 'cancel-change-password') { changingPassword = false; notice(null); render() }
    else if (action === 'sync-now') {
      if (syncPending) return
      syncPending = true
      notice('正在同步…', 'info')
      render()
      void window.electronAPI.gameAccountSyncNow().then((result) => {
        syncPending = false
        if (!result.ok) {
          applyResult(result)
          return
        }
        const feedback = syncFeedback(result.data)
        notice(feedback.text, feedback.tone)
        setState(result.data)
      }).catch(() => {
        syncPending = false
        notice('同步暂时不可用，稍后会自动重试。')
        render()
      })
    }
    else if (action === 'resolve-local') void window.electronAPI.gameAccountResolveConflict('local').then(applyResult).catch(() => { notice('暂时无法选择存档，请稍后重试。'); render() })
    else if (action === 'resolve-cloud') void window.electronAPI.gameAccountResolveConflict('cloud').then(applyResult).catch(() => { notice('暂时无法选择存档，请稍后重试。'); render() })
    else if (action === 'logout') void window.electronAPI.gameAccountLogout().then(applyResult).catch(() => { state = guestState(); notice('已退出当前账号，本地进度仍会保留。', 'info'); render() })
    else if (action === 'login') {
      const email = field('email')
      const password = field('password')
      if (!isValidEmail(email) || !isValidPassword(password)) { notice('请输入有效邮箱和至少 8 位密码。'); render(); return }
      void window.electronAPI.gameAccountLogin({ email, password }).then(applyResult).catch(() => { notice('登录暂时不可用，请稍后重试。'); render() })
      ...
      if (!isValidEmail(email) || !isValidCode(code) || !isValidPassword(newPassword)) { notice('请填写有效邮箱、六位验证码和至少 8 位新密码。'); render(); return }
      void window.electronAPI.gameAccountResetPassword({ email, code, newPassword }).then(applyResult).catch(() => { notice('重置密码暂时不可用，请稍后重试。'); render() })
    } else if (action === 'change-password') {
      const oldPassword = field('old-password')
      const newPassword = field('new-password')
      if (!oldPassword || !isValidPassword(newPassword)) { notice('请输入当前密码和至少 8 位新密码。'); render(); return }
      void window.electronAPI.gameAccountChangePassword({ oldPassword, newPassword }).then(applyResult).catch(() => { notice('修改密码暂时不可用，请稍后重试。'); render() })
    }
  })
